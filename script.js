...
async function importFiles(files) {
  ...
  for (let idx = 0; idx < allEntries.length; idx++) {
    ...
    if (trashIds.has(entry.id)) { skipped++; continue; }
...
async function createCard(item, context = 'general') {
  ...
  if (organizeMode) {
    ...
    arrows.style.cssText = 'position:absolute;top:8px;right:8px;display:flex;flex-direction:column;gap:3px;z-index:10;';
