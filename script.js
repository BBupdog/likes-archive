...
function parseCSV(text) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    results.push({
      id, date, username, displayName, text,
...
async function downloadAndStoreMedia(url, key) {
  try {
    ...
    return new Promise(resolve => {
      ...
      reader.onloadend = () => {
        dbPut('images', { id: key, data: reader.result, type: blob.type })
          .then(() => resolve(true));
      };
...
async function importFiles(files) {
  ...
  for (let idx = 0; idx < allEntries.length; idx++) {
    ...
    for (let i = 0; i < entry.images.length; i++) {
      ...
      if (ok) storedImages.push({ key, mediaType: 'image' });
    }
    ...
    if (entry.videoUrl) {
      ...
      if (ok) {
        storedImages.push({ key, mediaType: 'video' });
        ...
        if (thumbOk) storedImages.push({ key: thumbKey, mediaType: 'image' });
      }
    }
...
async function getFirstMediaElement(mediaKeys) {
  ...
  if (mediaKeys[0].mediaType === 'video') {
    ...
    video.muted = true;
    video.playsInline = true;
    video.style.cssText = 'width:100%;display:block;background:#000;min-height:60px;';
    return video;
  } else {
    const img = document.createElement('img');
    img.src = record.data;
    img.loading = 'lazy';
    img.style.cssText = 'width:100%;display:block;min-height:60px;';
    return img;
  }
}
  ...
  showConfirm(
    ...
    async () => {
      for (const id of ids) {
        ...
        if (like && like.mediaKeys) {
          for (const m of like.mediaKeys) await dbDelete('images', m.key);
        }
...
function refreshCurrentView() {
  ...
  if (currentTab === 'general') renderGallery(filter);
  else if (currentBoard) renderBoardGallery(currentBoard, filter);
  else renderBoards();
}
...
async function assignToBoard(boardName) {
  ...
  if (like) { like.board = boardName; await dbPut('likes', like); }
...
async function createNewBoard() {
  ...
  if (isMassMode) await assignMassToBoard(name);
  else await assignToBoard(name);
}
...
document.getElementById('wipe-log-btn').addEventListener('click', () => {
  showConfirm(
    ...
    'WIPE', 'Type WIPE to confirm',
    async () => { await dbClear('deleted'); alert('Delete log wiped successfully.'); }
  );
});
...
document.getElementById('search').addEventListener('input', e => {
  if (currentTab === 'general') renderGallery(e.target.value);
  else if (currentBoard) renderBoardGallery(currentBoard, e.target.value);
});
...
document.getElementById('lightbox-board-btn').addEventListener('click', () => {
  if (currentItem) { contextTargetItem = currentItem; openBoardPicker(false); }
});
